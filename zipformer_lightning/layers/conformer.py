...
from .subsampling import Conv2dSubsampling4, Conv2dSubsampling
from torch import Tensor, nn

from ..utils.icefall import is_jit_tracing, make_pad_mask, subsequent_chunk_mask
from .encoder import EncoderInterface

class Conformer(nn.Module, EncoderInterface):
    """
    Args:
    ...
    def __init__(
        ...
        if subsampling_layer == 'conv2d':
            self.encoder_embed = Conv2dSubsampling(num_features, d_model)
        else:
            self.encoder_embed = Conv2dSubsampling4(num_features, d_model)
        ...
        if pretrained_path and pretrained_path.strip():
            self.load_state_dict(torch.load(pretrained_path, map_location=next(self.parameters()).device)['state_dict'], strict=False)
    ...
    def forward(
        ...
        lengths = self.encoder_embed.compute_lengths(x_lens)
    ...
    @torch.jit.export
    def get_init_state(
        self, batch_size, left_context: int, device: torch.device
    ) -> List[torch.Tensor]:
    ...
    def streaming_forward(
        ...
        if not simulate_streaming:
            ...
            if isinstance(self.encoder_embed, Conv2dSubsampling):
